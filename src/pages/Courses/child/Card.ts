...
export default function Card(DOM: HTMLElement, StrandData: CourseType) {
  const courses = StrandData.map(course => {
    let difficultyClass = "";
    ...
    return useTSPurifier(
        /*html*/ `
            <div key=${
              course.id
            } class='group card relative overflow-hidden rounded-sm shadow-lg cursor-pointer'>
                ...
                <div class='relative overflow-hidden w-full h-[200px]'>
                    <img class='w-full h-full transition duration-500 ease-in-out transform group-hover:scale-150' src='${
                      course.image
                    }' alt='${course.name}' />
                </div>
                <div class='px-2 py-4'>
                    ...
                    <div class="absolute top-1 right-1 ${difficultyClass} py-0 px-2 rounded-sm">
                        <p class='text-[12px]'>${toCapitalized(
                          course.difficulty
                        )}</p>
                    </div>
