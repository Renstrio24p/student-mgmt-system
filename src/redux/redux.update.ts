...
export const updateUserData = createAsyncThunk(
  ...
  async ({
    ...
    _id: string;
    updateData: UserType[number];
  }) => {
    try {
      const res = await axiosInstance.post("/api/update?type=users", {
        _id,
        ...updateData,
      });
      alert(
        `${(res.statusText = `${res.data.data.name} record has been updated successfully..`)}`
      );
...
export const updateStudentData = createAsyncThunk(
  ...
  async ({
    ...
    _id: string;
    updateData: StudentType[number];
  }) => {
    try {
      const res = await axiosInstance.post("/api/update?type=student", {
        _id,
        ...updateData,
      });
      alert(
