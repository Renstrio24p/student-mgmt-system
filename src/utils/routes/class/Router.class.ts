type RouteCallback = (errorElement?: HTMLElement, params?: Record<string, string>) => void;
...
export class TSRouter {
    ...
    private handlePopState() {
        ...
        if (matchingRoute) {
            ...
            matchingRoute.element(errorElement, matchingRoute.params); // Pass params to element
            ...
            if (matchingRoute.children) {
                ...
                if (childElement) {
                    this.renderChildren(matchingRoute.children, nestedPath, childElement, matchingRoute.params!); // Pass params to children
                }
            }
        } else {
            ...
            if (notFoundRoute) {
                const errorElement = document.createElement('div');
                notFoundRoute.element(errorElement, notFoundRoute.params); // Pass params to element
            }
        }
    }

    private renderChildren(children: RouteConfig[] | undefined, nestedPath: string, parentElement: HTMLElement, parentParams: Record<string, string>) {
        ...
        if (matchingChild) {
            ...
            matchingChild.element(childElement, { ...parentParams, ...matchingChild.params }); // Merge parent and child params
            parentElement.appendChild(childElement);

            if (matchingChild.children) {
                const nextNestedPath = nestedPath.slice(matchingChild.path.length);
                this.renderChildren(matchingChild.children, nextNestedPath, childElement, { ...parentParams, ...matchingChild.params }); // Merge parent and child params
            }
        }
    }
